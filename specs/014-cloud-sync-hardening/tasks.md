...
- [ ] T1 [cloud_storage] — Add `cloud_key` free function
...
- [ ] T2 [core_types] — Add `SyncFailed` event variant to `SyncEvent`
  **File:** `core_types/src/events.rs`
  Add `SyncFailed { error: String }` variant to the `SyncEvent` enum.

- [ ] T3 [core_types] — Update `SyncStarted` to carry both upload and deletion counts
  **File:** `core_types/src/events.rs`
...
- [ ] T4 [database] — Add `CloudSyncableFileInfo` model; remove `generate_cloud_key` from `FileInfo`
...
- [ ] T5 [database] — Change `get_files_pending_upload` return type to `Vec<CloudSyncableFileInfo>`
...
- [ ] T6 [database] — Add split deletion queries
  **File:** `database/src/repository/file_info_repository.rs`
...
- [ ] T7 [service] — Update `SyncContext` for split deletion counts
...
- [ ] T8 [service] — Update `UploadPendingFilesStep`: new type, remove dead guards + lifecycle events, fix loop
...
- [ ] T9 [service] — Replace `DeleteMarkedFilesStep` with `DeleteCloudFilesStep` + `CleanupTombstonesStep`
  **File:** `service/src/cloud_sync/steps.rs`
...
- [ ] T10 [service] — Remove `GetSyncFileCountsStep` from pipeline and delete the step
...
- [ ] T11 [service] — Run counts + send `SyncStarted` in `service.rs` before pipeline
...
- [ ] T12 [service] — Send terminal events from `service.rs`
  **File:** `service/src/cloud_sync/service.rs`
...
- [ ] T13 [service] — Add `partial_successful_uploads` and `tombstones_cleaned_up` to `SyncResult`
  **File:** `service/src/cloud_sync/service.rs`
...
- [ ] T14 [service] — Update all remaining `generate_cloud_key` callers to `cloud_storage::cloud_key`
...
- [ ] T20 [service] — Test that upload loop tries all files when first batch all fail
...
- [ ] T21 [service] — Test that tombstones are cleaned up by `CleanupTombstonesStep`
  **File:** `service/src/cloud_sync/steps.rs` (test module)
...
- [ ] T22 [service] — Test that tombstone cleanup handles more than one batch
  **File:** `service/src/cloud_sync/steps.rs` (test module)
  Insert 11 tombstones. Run `CleanupTombstonesStep`. Verify all 11 are removed.

- [ ] T23 [service] — Test that `DeleteCloudFilesStep` loop tries all files when first batch all fail
  **File:** `service/src/cloud_sync/steps.rs` (test module)
  Analogous to T20 for cloud deletions: 15 files, first 10 fail, verify 11–15 are attempted.

- [ ] T24 [database] — Test `get_cloud_files_pending_deletion` and `get_tombstones_pending_deletion`
...
- [ ] T25 [service] — Test that `SyncCompleted` is sent even when no uploads are queued
...
- [ ] T26 [service] — Test that `SyncFailed` is sent on pipeline abort
  **File:** `service/src/cloud_sync/service.rs` or pipeline integration test
  Simulate a DB error in the count phase. Verify `SyncFailed` is sent.

- [ ] T27 [service] — Test that `partial_successful_uploads` is populated in `SyncResult`
  **File:** `service/src/cloud_sync/steps.rs` (test module)
  Simulate a successful cloud upload followed by a DB update failure. Verify
  `SyncResult.partial_successful_uploads == 1`.

- [ ] T28 [cloud_storage] — Test `cloud_key` free function
...
- [ ] T29 [service] — Test that `CleanupTombstonesStep` breaks out when all DB deletes fail
  **File:** `service/src/cloud_sync/steps.rs` (test module)
  Insert 5 tombstones. Configure the repository to fail `delete_file_info` for all of them
  (e.g. via a mock or by causing a constraint violation). Run `CleanupTombstonesStep`. Verify
  the step returns `StepAction::Continue` (does not hang), `context.tombstones_cleaned_up == 0`,
  and all 5 tombstones are still in `file_info`.

- [ ] T30 [service] — Test that `SyncCancelled` is sent exactly once and not from a step
  **File:** `service/src/cloud_sync/service.rs` or pipeline integration test
...
- [ ] T31 [service] — Test that `SyncStarted` carries correct upload and deletion counts
  **File:** `service/src/cloud_sync/service.rs` or pipeline integration test
...
- [ ] T33 [service] — Test that `SyncCompleted` is sent when there is nothing to sync
  **File:** `service/src/cloud_sync/service.rs` or pipeline integration test
...
- [ ] T34 [service] — Test `CleanupTombstonesStep::should_execute` returns false when no tombstones
  **File:** `service/src/cloud_sync/steps.rs` (test module)
...
- [ ] T35 [service] — Test `DeleteCloudFilesStep::should_execute` returns false when no cloud deletions
  **File:** `service/src/cloud_sync/steps.rs` (test module)
