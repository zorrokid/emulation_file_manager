...
pub trait FileMetadataOps: Send + Sync {
...
#[derive(Debug, Clone)]
pub struct StdFileMetadataOps;

impl FileMetadataOps for StdFileMetadataOps {
    fn read_zip_contents_with_checksums(
        &self,
        file_path: &Path,
    ) -> Result<HashMap<Sha1Checksum, ReadFile>, FileMetadataError> {
...
pub mod mock {
    ...
    impl FileMetadataOps for MockFileMetadataOps {
        fn read_zip_contents_with_checksums(
            &self,
            file_path: &Path,
        ) -> Result<HashMap<Sha1Checksum, ReadFile>, FileMetadataError> {
            if *self.should_fail.lock().unwrap() {
                Err(FileMetadataError::ZipError {
        ...
        fn read_file_checksum(
            ...
            if *self.should_fail.lock().unwrap() {
                return Err(FileMetadataError::FileIoError {
                    path: file_path.to_path_buf(),
                    message: "Mock error".to_string(),
                });
            }
            ...
            if map.is_empty() {
                return Err(FileMetadataError::FileIoError{
                        path: file_path.to_path_buf(),
                        message: "MockFileMetadataOps: no file checksums configured; call add_file_checksum() before use".to_string(),
                });
            }
            Ok(map.clone())
        }
